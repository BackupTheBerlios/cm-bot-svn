...
DT_double getAbsolutValue(const DT_vector * const v) {
	return sqrt(v->x * v->x + v->y * v->y);
}

void calculateMovementPoints(DT_vector * v) {
	...
	DT_point distV;
	if (distM <= distS) {
		if (getAbsolutValue(v) < distM) {
			ratio = distM / getAbsolutValue(v);
		...
		midS.z = Z;
		v->x = v->x - (isectM.x - pM.x);
		v->y = v->y - (isectM.y - pM.y);
	} else {
		if (getAbsolutValue(v) < distS) {
			ratio = distS / getAbsolutValue(v);
			distV.x = isectS.x - pS.x;
			distV.y = isectS.y - pS.y;
			isectS.x = pS.x + ratio * distV.x;
			isectS.y = pS.y + ratio * distV.y;
		...
		midM.y = (pM.y + isectM.y) / 2;
		midM.z = Z;
		v->x = v->x - (isectS.x - pS.x);
		v->y = v->y - (isectS.y - pS.y);
	}
}
...
DT_bool movedToPoint(DT_vector * v) {
	DT_bool movedToPoint = true;
	if (round(v->x) > -10 && 10 > round(v->x)) {
		movedToPoint = false;
	}
	if (round(v->y) > -10 && 10 > round(v->y)) {
		movedToPoint = false;
	}
	return movedToPoint;
}
